...
class Controller {
  ...
  checkmate() {
    const opponentTiles = this.game.board.getTilesByPieceColor(this.opponentColor)
  ...
  currentCheck(): boolean {
    ...
    const opponentTiles = this.game.board.getTilesByPieceColor(this.opponentColor)
    for (const opponentTile of opponentTiles) {
      if (opponentTile.piece?.canMove(opponentTile, this.game.board.getKingTile(this.currentColor))) {
        return true
      }
    }
    return false
  ...
  opponentCheck(): boolean {
    ...
    const currentTiles = this.game.board.getTilesByPieceColor(this.currentColor)
    for (const currentTile of currentTiles) {
      if (currentTile.piece?.canMove(currentTile, this.game.board.getKingTile(this.opponentColor))) {
        return true
      }
    }
    return false
  ...
  move(player: Player, start: Position, end: Position): boolean {
    ...
    if ((this.currentPlayer == this.game.whitePlayer && this.game.board.getTile(start.row, start.column)?.getPieceColor() != Color.WHITE) || (this.currentPlayer == this.game.blackPlayer && this.game.board.getTile(start.row, start.column)?.getPieceColor() != Color.BLACK)) {
      return false
    }
    ...
    if (this.pieceCanMove(startTile, endTile)) {
      ...
      if (this.opponentCheck()) {
        ...
        if (this.checkmate()) {
          ...
          this.game.state = this.currentColor === Color.WHITE ? State.WHITE_WON : State.BLACK_WON
          return true
        }
      ...
      this.currentPlayer = this.currentPlayer === this.game.whitePlayer ? this.game.blackPlayer : this.game.whitePlayer
